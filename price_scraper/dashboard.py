...
df["cheaper"] = df.apply(
    lambda row: "Links" if row["links_price"] < row["instar_price"]
    else "Instar" if row["instar_price"] < row["links_price"]
    else "Same price",
    axis=1
)
...
app.layout = html.Div([
    ...
    html.H2(f"Links cheaper: {links_cheaper}"),
    html.H2(f"Instar cheaper: {instar_cheaper}"),
    ...
    dcc.Dropdown(
        ...
        options=[
            ...
            {"label": "Links cheaper", "value": "Links"},
            {"label": "Instar cheaper", "value": "Instar"},
